...
| Six residual Dependabot alerts | **When Next.js bumps its own pins** | Alerts went 83 → 6 on 2026-08-10 (`next` 16.2.0 → 16.2.11 cleared all 46 of its own). The remainder are `postcss 8.4.31` and `sharp 0.34.5`, both pinned by `next@16.2.11` itself, and `esbuild 0.27.7` pinned by `tsup`. Deliberately **not** forced via `pnpm.overrides` — postcss and sharp are load-bearing for Next's build and image optimization, and the exposure is nil: `apps/docs` is private and undeployed, `tsup` is a devDependency, and none of the three appear in the published CLI's production tree |
