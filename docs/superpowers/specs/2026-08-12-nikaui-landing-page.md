...
- **The motion section** renders `Card` five times, each wrapped in `NikaMotionConfig` at a different preset, with a replay control. A visitor sees the actual shipped physics rather than a CSS approximation of it.

  **Corrected 2026-08-12, during Task 7.** This paragraph originally justified `Card` on the grounds that it expresses "both halves of a preset — the spring configuration through its hover scale, and the travel multiplier through its entrance." That was wrong about the component: `Card` has no hover scale. It reads `feel.transition`, `travel` and `enabled`, and never touches `feel.scale`. So the section demonstrates three of the four `MotionFeel` fields — settle physics, entrance distance, and `none` as a clean off state — and the `scale.hover` / `scale.tap` half is not shown anywhere on the page. `Button` is the component that reads `scale`, and the one `Button` in the section (Replay) sits outside the per-preset providers, so it reflects only the ambient preset.

  `Card` remains the right subject: four presets show visibly different settle physics and travel distances, and `none` shows the off state. But the choice was made for a stated reason that did not hold, and sub-project D should know that before it writes documentation repeating it. Adding a per-preset `Button` beside each `Card` would close the gap; it was not done here because it was not in the approved scope.
